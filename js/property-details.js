class PropertyDetails {
    ...
    async loadPropertyDetails() {
        try {
            ...
            const detailsResponse = await fetch(`${this.apiUrl}?id=${this.propertyId}`);
            const propertyData = await detailsResponse.json();

            // Load property images
            const imagesResponse = await fetch(`../api/properties/get_images.php?property_id=${this.propertyId}`);
            const imagesData = await imagesResponse.json();

            if (detailsResponse.ok) {
                // Add images to property data
                propertyData.images = imagesData.images.map(img => img.url);
                this.updatePropertyDetails(propertyData);
            } else {
                this.showError(propertyData.message);
            }
        } catch (error) {
